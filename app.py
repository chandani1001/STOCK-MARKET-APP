...
# ── Custom Pink CSS Theme ─────────────────────────────────────
st.markdown("""
    <style>
    .stApp { background-color: #fff0f5; }
    [data-testid="stSidebar"] { background-color: #ffe4ef; }
    h1 { color: #c0006a; font-family: Arial, sans-serif; font-weight: bold; }
    h2, h3 { color: #d63384; font-family: Arial, sans-serif; }
    [data-testid="stMetric"] { background-color: #ffe4ef; border: 1px solid #f5a7c7; border-radius: 10px; padding: 10px; }
    hr { border-color: #f5a7c7; }
    [data-testid="stDataFrame"] { border: 1px solid #f5a7c7; border-radius: 8px; }
    .section-box { background-color: #fff5f8; border-left: 4px solid #d63384; padding: 10px 15px; border-radius: 5px; margin-bottom: 10px; }
    </style>
...
stocks_full = [
    ('AAPL', aapl_full, '#e75480'),
    ('MSFT', msft_full, '#c0006a'),
    ('NFLX', nflx_full, '#ff85a1'),
    ('GOOG', goog_full, '#ff1493')
]
...
if selected_ticker == 'All':
    ...
    color_map = {'AAPL': '#e75480', 'MSFT': '#c0006a', 'NFLX': '#ff85a1', 'GOOG': '#ff1493'}
...
# ── Header ────────────────────────────────────────────────────
st.markdown("""
    <h1 style='text-align:center;'>📈 Stock Market Analysis Dashboard</h1>
    <p style='text-align:center; color:#d63384; font-size:16px;'>
    Apple (AAPL) &nbsp;|&nbsp; Microsoft (MSFT) &nbsp;|&nbsp; Netflix (NFLX) &nbsp;|&nbsp; Google (GOOG)<br>
    February 2023 — May 2023 &nbsp;|&nbsp; Prepared by: <b>Chandani Sah</b> | Oracle Brain Internship
    </p>
""", unsafe_allow_html=True)
...
st.markdown('<div class="section-box">Closing price movement over the selected period. Shows overall direction — upward, downward or flat.</div>', unsafe_allow_html=True)
...
if len(active_stocks) == 1:
    ...
    if len(data) > 1:
        ...
        ax.set_title(ticker + ' — Closing Price Trend', fontsize=14, fontweight='bold', color='#c0006a')
        ax.set_xlabel('Date')
    ...
    fig.patch.set_facecolor('#fff0f5')
    fig.suptitle('Closing Price Trends', fontsize=16, fontweight='bold', color='#c0006a')
    axes_flat = axes.flatten()
    for i, (ticker, data, color) in enumerate(active_stocks):
        if len(data) > 1:
            axes_flat[i].set_facecolor('#fff5f8')
            ...
            axes_flat[i].set_title(ticker, fontweight='bold', color='#c0006a')
...
if selected_ticker == 'All':
    ...
    ax.set_title('Normalized Price Comparison', fontsize=14, fontweight='bold', color='#c0006a')
    ax.set_xlabel('Date')
...
st.markdown('<div class="section-box">Daily trading volume shows how actively each stock is being bought and sold. Spikes often signal important market events.</div>', unsafe_allow_html=True)
...
fig.patch.set_facecolor('#fff0f5')
fig.suptitle('Daily Trading Volume', fontsize=16, fontweight='bold', color='#c0006a')
axes_flat = [axes] if n == 1 else axes.flatten()
for i, (ticker, data, color) in enumerate(active_stocks):
    if len(data) > 1:
        axes_flat[i].set_facecolor('#fff5f8')
        axes_flat[i].bar(data['Date'], data['Volume'] / 1e6, color=color, alpha=0.75, width=0.8)
        axes_flat[i].set_title(ticker + ' — Avg: ' + str(round(data['Volume'].mean() / 1e6, 1)) + 'M', fontweight='bold', color='#c0006a')
        axes_flat[i].set_xlabel('Date')
...
st.markdown('<div class="section-box">Moving averages smooth out daily price noise to reveal the underlying trend. When price stays above MA20 it confirms an uptrend.</div>', unsafe_allow_html=True)
...
fig, axes = plt.subplots(nrows, ncols, figsize=(16, nrows * 4))
fig.patch.set_facecolor('#fff0f5')
fig.suptitle('Moving Averages — 20-Day & 50-Day', fontsize=16, fontweight='bold', color='#c0006a')
axes_flat = [axes] if n == 1 else axes.flatten()
for i, (ticker, data, color) in enumerate(active_stocks):
    if len(data) > 1:
        axes_flat[i].set_facecolor('#fff5f8')
        axes_flat[i].plot(data['Date'], data['Close'], color=color,     alpha=0.5, linewidth=1.5, label='Close')
        axes_flat[i].plot(data['Date'], data['MA20'],  color='#8b0000', linewidth=2, linestyle='--', label='MA20')
        axes_flat[i].plot(data['Date'], data['MA50'],  color='#4b0082', linewidth=2, linestyle='-.', label='MA50')
        axes_flat[i].set_title(ticker, fontweight='bold', color='#c0006a')
        axes_flat[i].set_xlabel('Date')
        axes_flat[i].set_ylabel('Price (USD)')
...
st.markdown('<div class="section-box">Volatility measures how wildly a stock price jumps around day to day. Higher volatility means higher risk.</div>', unsafe_allow_html=True)
...
with col_left:
    ...
    fig.patch.set_facecolor('#fff0f5')
    fig.suptitle('Daily Returns', fontsize=14, fontweight='bold', color='#c0006a')
    axes_flat = [axes] if n == 1 else axes.flatten()
    for i, (ticker, data, color) in enumerate(active_stocks):
        if len(data) > 1:
            axes_flat[i].set_facecolor('#fff5f8')
...
with col_right:
    ...
    if returns_dict:
        ...
        pink_palette   = ['#e75480', '#c0006a', '#ff85a1', '#ff1493']
        fig, ax = plt.subplots(figsize=(8, 6))
        fig.patch.set_facecolor('#fff0f5')
        ax.set_facecolor('#fff5f8')
        sns.violinplot(data=returns_melted, x='Stock', y='Daily Return (%)', palette=pink_palette[:len(returns_dict)], inner='box', ax=ax)
        ax.axhline(y=0, color='black', linestyle='--', linewidth=0.8)
        ax.set_title('Return Distribution', fontweight='bold', color='#c0006a')
        plt.tight_layout()
        st.pyplot(fig)
    ...
    for ticker, data, color in active_stocks:
        if len(data) > 1 and 'Daily_Return' in data.columns:
            ...
            vol_rows.append({'Stock': ticker, 'Avg Return (%)': round(data['Daily_Return'].mean(), 3), 'Volatility (%)': round(std, 3), 'Best Day (%)': round(data['Daily_Return'].max(), 2), 'Worst Day (%)': round(data['Daily_Return'].min(), 2), 'Risk': risk})
    if vol_rows:
...
st.markdown('<div class="section-box">Correlation shows how much two stocks move together. Close to +1 means they move in sync. Close to 0 means they move independently.</div>', unsafe_allow_html=True)
...
with col_left:
    ...
    fig.patch.set_facecolor('#fff0f5')
    ax.set_facecolor('#fff5f8')
    pink_cmap = sns.diverging_palette(340, 10, as_cmap=True)
    sns.heatmap(corr_matrix, annot=True, fmt='.3f', cmap=pink_cmap, linewidths=0.5, square=True, vmin=-1, vmax=1, ax=ax)
    ax.set_title('Correlation Heatmap', fontweight='bold', color='#c0006a')
    plt.tight_layout()
    st.pyplot(fig)
...
with col_right:
    ...
    fig.patch.set_facecolor('#fff0f5')
    pd.plotting.scatter_matrix(close_pivot, figsize=(7, 5), diagonal='kde', alpha=0.5, color='#e75480', ax=axes)
    plt.suptitle('Scatter Matrix', fontsize=12, fontweight='bold', color='#c0006a')
    plt.tight_layout()
    st.pyplot(fig)
...
st.markdown('<div class="section-box">Linear Regression and Random Forest models trained to predict next day closing price. Evaluated using R² Score and RMSE.</div>', unsafe_allow_html=True)
...
for ticker, res in results.items():
    eval_rows.append({'Stock': ticker, 'LR R2': res['lr_r2'], 'LR RMSE ($)': res['lr_rmse'], 'RF R2': res['rf_r2'], 'RF RMSE ($)': res['rf_rmse'], 'Better Model': 'Linear Regression' if res['lr_r2'] > res['rf_r2'] else 'Random Forest'})
...
if active_results:
    ...
    fig, axes = plt.subplots(nrows, ncols, figsize=(16, nrows * 4))
    fig.patch.set_facecolor('#fff0f5')
    fig.suptitle('Predicted vs Actual Closing Price', fontsize=16, fontweight='bold', color='#c0006a')
    axes_flat = [axes] if n == 1 else axes.flatten()
    for i, (ticker, res) in enumerate(active_results.items()):
        ...
        axes_flat[i].plot(x_axis, res['lr_pred'], color='#8b0000', linewidth=1.5, linestyle='--', label='LR')
        axes_flat[i].plot(x_axis, res['rf_pred'], color='#4b0082', linewidth=1.5, linestyle=':', label='RF')
        axes_flat[i].set_title(ticker + ' | LR R2: ' + str(res['lr_r2']) + ' | RF R2: ' + str(res['rf_r2']), fontweight='bold', color='#c0006a')
...
for ticker, data, color in stocks_full:
    if len(data) > 1:
        ...
        summary.append({'Stock': ticker, 'Start Price ($)': round(start, 2), 'End Price ($)': round(end, 2), '3M Return (%)': round(change, 2), 'Period High ($)': round(data['Close'].max(), 2), 'Period Low ($)': round(data['Close'].min(), 2), 'Volatility (%)': round(data['Daily_Return'].std(), 2), 'Trend': 'Positive' if change > 0 else 'Negative'})
st.dataframe(pd.DataFrame(summary), use_container_width=True)

st.markdown('---')
st.markdown('<p style="text-align:center; color:#d63384;">📈 Stock Market Analysis Dashboard &nbsp;|&nbsp; Chandani Sah &nbsp;|&nbsp; Data Analyst Intern &nbsp;|&nbsp; Oracle Brain, New Baneshwor &nbsp;|&nbsp; March 2025</p>', unsafe_allow_html=True)